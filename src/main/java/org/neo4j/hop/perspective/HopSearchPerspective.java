...
import org.neo4j.hop.logging.Defaults;
import org.neo4j.hop.logging.util.LoggingCore;
...
public class HopSearchPerspective implements IHopPerspective {
  ...
  @Override public void initialize( HopGui hopGui, Composite parent ) {
    ...
    neo4jImage = SwtSvgImageUtil.getImage( hopGui.getDisplay(), this.getClass().getClassLoader(), "neo4j_logo.svg", size, size );
    lineageImage = SwtSvgImageUtil.getImage( hopGui.getDisplay(), this.getClass().getClassLoader(), "lineage.svg", size, size );
  ...
  private void analyze( Event event ) {
    ...
    try {
      ...
      try {
        ...
        analyzeCypherStatements( connection, session, id, name, type, errors );
  ...
  private void analyzeErrorLineage( Session session, String id, String name, String type, int errors ) {
  ...
  private void analyzeCypherStatements( NeoConnection connection, Session session, String id, String name, String type, int errors ) {
    ...
    cypher.append( "Below are a few Cypher statements you can run in the Neo4j browser" ).append( Const.CR );
