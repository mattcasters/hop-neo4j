...
public class MetadataUpdateDialog extends ActionDialog implements IActionDialog {

  public static final String CHECK_CONNECTIONS_DIALOG = "Neo4jCheckConnectionsDialog";
  private static Class<?> PKG = MetadataUpdateDialog.class; // for i18n purposes, needed by Translator2!!
  ...
  public MetadataUpdateDialog( Shell parent, IAction iAction, WorkflowMeta workflowMeta ) {
    super( parent, workflowMeta );
    this.metadataUpdate = (MetadataUpdate) iAction;

    if ( this.metadataUpdate.getName() == null ) {
      this.metadataUpdate.setName( "Neo4j Cypher Script" );
    }
  }
  ...
  @Override public IAction open() {
    ...
    props.setLook( shell );
    WorkflowDialog.setShellImage( shell, metadataUpdate );

    ModifyListener lsMod = new ModifyListener() {
      public void modifyText( ModifyEvent e ) {
        metadataUpdate.setChanged();
      }
    };
    changed = metadataUpdate.hasChanged();
  ...
  private void cancel() {
    metadataUpdate.setChanged( changed );
    metadataUpdate = null;
    dispose();
  }

  private void getData() {
    wName.setText( Const.NVL( metadataUpdate.getName(), "" ) );
    wConnection.setText( Const.NVL( metadataUpdate.getConnectionName(), "" ) );
    wScript.setText( Const.NVL( metadataUpdate.getScript(), "" ) );
    wReplaceVariables.setSelection( metadataUpdate.isReplacingVariables() );
  }
  ...
  private void ok() {
    ...
    metadataUpdate.setName( wName.getText() );
    metadataUpdate.setConnectionName( wConnection.getText() );
    metadataUpdate.setScript( wScript.getText() );
    metadataUpdate.setReplacingVariables( wReplaceVariables.getSelection() );
